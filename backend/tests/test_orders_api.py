...
async def test_get_orders_empty_200(client):
...
async def test_post_order_201_and_list(client):
...
async def test_post_order_invalid_term_422(client):
...
async def test_post_order_no_yield_404(client):
...
async def test_patch_order_settled_200(client):
    ...
    with patch(
        ...
        create_resp = await client.post("/api/order", json={"term": "10Y", "amount": 5000})
    assert create_resp.status_code == 201
    order_id = create_resp.json()["id"]

    patch_resp = await client.patch(f"/api/orders/{order_id}", json={"status": "Settled"})
...
async def test_patch_order_failed_200(client):
    ...
    with patch(
        ...
        create_resp = await client.post("/api/order", json={"term": "2Y", "amount": 1000})
    assert create_resp.status_code == 201
    order_id = create_resp.json()["id"]

    patch_resp = await client.patch(f"/api/orders/{order_id}", json={"status": "Failed"})
    assert patch_resp.status_code == 200
    assert patch_resp.json()["status"] == "Failed"


async def test_patch_order_not_found_404(client):
    """PATCH /api/orders/99999 returns 404."""
    _clear_orders()
    response = await client.patch("/api/orders/99999", json={"status": "Settled"})
    assert response.status_code == 404


async def test_patch_order_invalid_transition_400(client):
    ...
    with patch(
        ...
        create_resp = await client.post("/api/order", json={"term": "5Y", "amount": 1000})
    order_id = create_resp.json()["id"]
    await client.patch(f"/api/orders/{order_id}", json={"status": "Settled"})

    # Now try to change again
    response = await client.patch(f"/api/orders/{order_id}", json={"status": "Failed"})
